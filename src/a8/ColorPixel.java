...
public class ColorPixel implements Pixel {
	...
	public Pixel blend(Pixel p, double w){
		if((w > 1) || (w < 0)){
			throw new RuntimeException("Weight is out of bounds. " + w);
		}
		newred = (red*w) + p.getRed()*(1-w);
		newblue = (blue*w) + p.getBlue()*(1-w);
		newgreen = (green*w) + p.getGreen()*(1-w);
		Pixel blendedpixel = new ColorPixel(newred, newgreen, newblue);
		return blendedpixel;
	}
